...
//resets all values
function reset() {
  if (confirm("Are you sure that you got stuffed and want to beat everything ???") === true) {
    money = 0;
    moneyup = 1;
    msec = 0;
    upcost = 15;
...
function myTimer() {
    money += msec;
  document.getElementById("total").innerHTML = "👊hIT: 👊" + addcomma(money);
}
...
function upgrade(name) {
  if (name == "clicker cat") {
    if (money >= catcost && catown < 50) {
      ...
      catcost = catcost * 2;
      document.getElementById("cat").innerHTML =
        catown + "👊Hit: " + addcomma(catcost) + " | +" + addcomma(catadd * cboost) + "/sec";
    } else if (catown == 50) {
      document.getElementById("cat").innerHTML =
        catown + " 👊HIT: MAX | +15% hit/sec👊";
    }
  }
  ...
  if (name == "worker") {
    if (money >= workercost && workerown < 50) {
      ...
      document.getElementById("worker").innerHTML = 
        workerown + "👊-workerHit:   👊" + addcomma(workercost) + " | +" + addcomma(workadd * wboost) + "/sec";
    } else if (workerown == 50) {
      document.getElementById("worker").innerHTML =
        workerown + "👊-worker-HIT: MAX | +35% click/sec👊";
    }
  }
  ...
  document.getElementById("click").innerHTML =
    "👊Hit: " + addcomma(moneyup) + " | 👊Hit/sec: " + addcomma(msec);
  document.getElementById("total").innerHTML = "👊HIT: " + addcomma(money);
}
...
$(".upload-button").on('ENTER',function(){
$(".file-upload").ENTER();
});
